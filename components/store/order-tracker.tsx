  ...
  return (
    <main className="sf-canvas min-h-screen px-5 py-10 sm:px-8 sm:py-14">
      <div className="mx-auto w-full max-w-3xl">
        ...
        {loading && !order && (
          <div className="flex items-center gap-3 rounded-xl border border-line bg-white p-6 text-sm" style={{ color: 'var(--sf-muted)' }}>
        ...
        {error && !order && (
          <div className="rounded-xl border border-line bg-white p-6">
            <p className="text-sm" style={{ color: 'var(--sf-heading)' }}>
              {error}
            </p>
        ...
        {order && (
          <div className="animate-rise-in flex flex-col gap-5">
                      ...
                      <li key={status} className="flex gap-3.5">
                        <div className="flex flex-col items-center">
                          <span
                            ...
                            style={
                              reached
                                ? { borderColor: 'var(--sf-gold-line)', background: 'var(--sf-gold-wash)', color: 'var(--sf-maroon)' }
                                : { borderColor: 'var(--sf-line)', color: 'var(--sf-muted)' }
                            }
                          >
                          ...
                          {index < ORDER_TIMELINE.length - 1 && (
                            <span
                              ...
                              style={{ background: index < stepIndex ? 'var(--sf-gold-line)' : 'var(--sf-line)' }}
                              aria-hidden
                            />
                          )}
                        </div>
                        <div className={`pb-6 ${current ? '' : 'opacity-70'}`}>
                          <p className="text-sm font-medium" style={{ color: 'var(--sf-heading)' }}>
            ...
            <section className="sf-card p-6">
              <h2 className="text-sm font-semibold tracking-wide">Your pieces</h2>
              <ul className="mt-4 flex flex-col divide-y divide-line">
                {order.lines.map((line) => (
                  <li key={line.id} className="flex items-center justify-between gap-4 py-3">
                    <div className="min-w-0">
                      <p className="truncate text-sm" style={{ color: 'var(--sf-heading)' }}>
                        {line.itemName}
                      </p>
                      <p className="text-[11px] tracking-wide uppercase" style={{ color: 'var(--sf-muted)' }}>
                        {line.category} · #{line.itemCode} · ×{line.quantity}
                      </p>
                    </div>
                    <p className="tnum shrink-0 text-sm">{rupeesExact(Number(line.lineTotal))}</p>
                  </li>
                ))}
              </ul>
              <div className="mt-4 flex items-center justify-between border-t border-line pt-4">
                <span className="text-sm">
                  Total ({order.itemCount} {order.itemCount === 1 ? 'piece' : 'pieces'})
                </span>
                <span className="tnum text-lg font-semibold" style={{ color: 'var(--sf-maroon)' }}>
                  {rupeesExact(Number(order.totalAmount))}
                </span>
              </div>
            </section>
            ...
            <section className="grid gap-4 sm:grid-cols-2">
              <div className="sf-card p-5">
                ...
                >
                  {order.fulfilment === 'DELIVERY' ? (
                    <MapPin className="size-3.5" style={{ color: 'var(--sf-gold-deep)' }} />
                  ) : (
                    <Store className="size-3.5" style={{ color: 'var(--sf-gold-deep)' }} />
                  )}
                  {order.fulfilment === 'DELIVERY' ? 'Home delivery' : 'Store pickup'}
                </p>
                <p className="mt-2 text-sm leading-6" style={{ color: 'var(--sf-heading)' }}>
                  {order.shop.name}
                </p>
                {order.shop.address && (
                  <p className="mt-1 text-xs leading-5" style={{ color: 'var(--sf-muted)' }}>
                    {order.shop.address}
                  </p>
                )}
              </div>
              ...
              <div className="sf-card p-5">
                <p className="text-[11px] font-semibold tracking-[0.14em] uppercase" style={{ color: 'var(--sf-heading)' }}>
            ...
            <p className="flex items-center gap-2 text-[11px]" style={{ color: 'var(--sf-muted)' }}>
